...
const AddList = ({
	...
	const getTodoList = (workSpaceId: any) => {
	...
	useEffect(() => {
		props?.currentModal?.modalName === "ADD_TODO_MODAL" &&
			getTodoList(props?.currentFirm?._id);
	}, []);
	...
	const addList = (e: any) => {
		...
		if (name !== "") {
			setLogging(true);
			agent.Todo.addList({ name, workSpaceId })
				.then((response: any) => {
					setLogging(false);
					getTodoList(props.params?.firmId);
					dispatch(
	...
	const todoTemp = {
		toDo: "",
		workSpaceId: props?.currentFirm?._id,
		toDoListId: "",
		description: "",
		date: new Date(),
		star: false,
