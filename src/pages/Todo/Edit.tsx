...
function EditTodoForm(props: FormProps & PropsFromRedux) {
  const { todo, visibility, setVisibility, editTodo, getTodos } = props;

  const [logging, setLogging] = useState(false);

  const editTodoTemp: any = {
    toDoId: todo._id,
  ...
  return (
    <form
      className={`${
        visibility.form ? "flex" : "hidden"
      } w-full p-2 bg-transparent bg-white flex-col sm:items-center`}
      onSubmit={onSubmit}
    >
      <div className="w-full group relative px-4 py-3 sm:p-3">
        <div className="flex gap-4">
          ...
          />
          <div
            className={`hidden group-focus-within:flex absolute right-[4.5rem]`}
          >
            <input
              type="date"
              name="reminderDate"
              id={`reminderDate-${editTemp?.toDoId}`}
              value={formatDate(editTemp?.reminderDate, true)}
              onChange={onChange}
          ...
          <button
            className="hidden group-focus-within:grid place-items-center  rounded-md px-2 border-[1px] border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-50"
