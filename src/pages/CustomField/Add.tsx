...
type Props = {
  ...
  customFieldData?: (newField: any) => void;
  from?: string;
};
...
class AddCustomField extends React.Component<Props, PropsFromRedux> {
  state: {
    logging: boolean;
    name: string;
    type: string;
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      name: "",
      type: "",
      applicableFor: this.props.from ? this.props.from : "",
      description: "",
    };
  }
  ...
  addCustomField = () => {
    ...
    if (name !== "" && type !== "") {
      ...
      agent.CustomField.addCustomField(
        name,
        description,
        type,
        applicableFor,
        ...
        .then((response: any) => {
          ...
          if (
            this.props.showFieldModal === true &&
            (applicableFor === "contactperson" ||
              applicableFor === "client" ||
              applicableFor === "task")
          ) {
            const newField = {
            ...
            this.sendCustomField(newField);
          }
          (this.props as any).addNotification(
            "Custom Field Added",
            "Successfully added a field.",
            "success"
          );
  ...
  closeFieldModal = (fetchAgain: boolean) => {
    this.props.closeModal(fetchAgain);
  };

  sendCustomField = (newField: any) => {
    this.props.customFieldData!(newField);
  };
        ...
        <Transition.Root
          show={
            (this.props as any)?.currentModal?.modalName ===
              "ADD_CUSTOM_FIELD_MODAL" || this.props.showFieldModal
          }
          as={Fragment}
        >
          <Dialog
...
export default connector(AddCustomField);
