...
export default function validGSTIN(gstin: string) {
  let upperCaseGstin = gstin.toUpperCase();
  var isValidFormat = false;
  if (checkPattern(upperCaseGstin, GSTINFORMAT_REGEX)) {
    isValidFormat = verifyCheckDigit(upperCaseGstin);
  }
...
function checkPattern(inputval: any, regxpatrn: any) {
  var result = false;
...
function getGSTINWithCheckDigit(gstinWOCheckDigit: any) {
  var factor = 2;
  var sum = 0;
